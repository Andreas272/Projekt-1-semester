...
void plane_reader (int time_array[array_size][array_size],int size_of_array) {
    ...
    // Here the file is opened.
    in_file = fopen("data_2.txt", "r"); // Opens the file data.txt in read only mode.
    ...
    if (in_file == NULL) {  //Tests if the file exists, and exit if it doesn't
        ...
        printf("\nThe file was found, you are flying son!!!\n");
        ...
        while(fscanf(in_file, "%d,%d,%d",&start,&end, &time) != EOF){
            time_array[start-1][end-1] = time + 115; //Have to -1, because the array starts at 0 and not 1. 115 is hidden time for domestic routes.
        }
